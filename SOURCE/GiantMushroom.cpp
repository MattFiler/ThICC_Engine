...
void GiantMushroom::InitMushroomData()
{
	...
	m_growthData.m_sizeChangeDuration = (float)m_itemData["MUSHROOM_GIANT"]["info"]["grow"]["growth_duration"];*/
}
...
void GiantMushroom::Tick()
{
	...
	if (m_itemUsed)
	{
		...
		case ItemGrowthData::GROW:
			m_player->SetScale(Vector3::Lerp(m_growthData.m_shrinkScale, m_growthData.m_growScale, m_growthData.m_scalePercent));
		...
		case ItemGrowthData::MAINTIAIN:
			...
			m_player->setInvicible(m_growthData.m_timeElapsed < m_growthData.m_sizeChangeDuration);
			m_growthData.m_timeElapsed += Locator::getGSD()->m_dt;
			if (m_growthData.m_timeElapsed >= m_growthData.m_sizeChangeDuration)
			{
