...
void Camera::Tick(GameStateData* _GSD)
{
	...
	case BEHAVIOUR::ORBIT:
		...
		angle -= 0.4f;
		Vector3 orbit_pos;
		orbit_pos.x = sin(angle / 57.2958f) * std::sqrt((base_pos.x - m_targetObject->GetPos().x) *  (base_pos.x - m_targetObject->GetPos().x));
		orbit_pos.z = cos(angle / 57.2958f) * std::sqrt((base_pos.z - m_targetObject->GetPos().z) *  (base_pos.z - m_targetObject->GetPos().z));
